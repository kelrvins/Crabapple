    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
          result += `<div class="node"><span class="node-title">
          <a href="/article/${data[i].articleID}">${data[i].title}</a>
        <div class="node-bg"></div></span><p class="node-content">${data[i].contents}</p>
        <div class="node-info"><span class="node-time">${data[i].res}</span>
        <span class="article-opera"><a id="modifyArticle" class="modify-article" href="${data[i].articleID}">编辑</a>
        <span id="delArticle" class="del-article" data-id="${data[i].articleID}">删除</span>
        </span><span class="node-tags">`
